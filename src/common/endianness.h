...
#if !defined(PHAETHON_NEED_ALIGNMENT)
...
// use software fallback by loading each byte explicitly
	...
	#if defined(PHAETHON_LITTLE_ENDIAN)
	...
	#elif defined(PHAETHON_BIG_ENDIAN)
...
// if the unaligned load and the byteswap take a lot instructions its better to directly read and invert
	#if defined(PHAETHON_NEED_ALIGNMENT) && !defined(__mips__)
	...
	#endif // if defined(PHAETHON_NEED_ALIGNMENT)
...
#endif // if defined(PHAETHON_LITTLE_ENDIAN)
