...
/** Return the number of bytes necessary to hold an image of these dimensions
  * and in this format. */
static inline uint32_t getDataSize(PixelFormat format, int32_t width, int32_t height) {
...
static inline bool hasValidDimensions(PixelFormat format, int32_t width, int32_t height) {
	if ((width < 0) || (width >= 0x8000) || (height < 0) || (height >= 0x8000))
		return false;
