...
struct ClassInfoGenerator {
    State& s;
    constexpr ClassInfoGenerator(State& st) : s(st) {}
...
struct MethodGenerator {
    ...
    constexpr MethodGenerator(State& st, int pi) : s(st), parameterIndex(pi) {}
...
struct PropertyGenerator {
    State& s;
    constexpr PropertyGenerator(State& st) : s(st) {}
...
struct NotifySignalGenerator<State, L, T, true> {
    ...
    using TP = T**;
    State& s;
    constexpr NotifySignalGenerator(State& st) : s(st) {}
...
struct EnumGenerator {
    ...
    constexpr EnumGenerator(State& st, int di) : s(st), dataIndex(di) {}
...
struct EnumValuesGenerator {
    State& s;
    constexpr EnumValuesGenerator(State& st) : s(st) {}
...
struct MethodParametersGenerator {
    State& s;
    constexpr MethodParametersGenerator(State& st) : s(st) {}
    template<class Method, class Index>
    constexpr void operator() (const Method& method, Index) {
...
struct ConstructorParametersGenerator {
    State& s;
    constexpr ConstructorParametersGenerator(State& st) : s(st) {}
